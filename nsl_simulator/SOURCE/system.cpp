...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising 1D 
    ...
    double prob_spin = 1.0/(1.0+exp(-_beta*delta_E));// prob spin to be 1 
    if(_rnd.Rannyu() > prob_spin) {_particle(i).setspin(-1);
...
void System :: initialize(string path){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory

  int p1, p2; // Read from ../INPUT/Primes a pair of numbers to be used to initialize the RNG
  output_path = path; // setting output path 
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      ...
      if(_sim_type == 0)      coutf << "LJ MOLECULAR DYNAMICS (NVE) SIMULATION"  << endl;// ex4
      else if(_sim_type == 1) coutf << "LJ MONTE CARLO (NVT) SIMULATION"         << endl;// ex7
      else if(_sim_type == 2) coutf << "ISING 1D MONTE CARLO (MRT^2) SIMULATION" << endl;//ex6
      else if(_sim_type == 3) coutf << "ISING 1D MONTE CARLO (GIBBS) SIMULATION" << endl;//ex6
...
void System :: initialize_velocities(){
  if(_restart and _sim_type==0){// if sim is restarted use last velocities
    ifstream cinf;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        if(_sim_type == 1){
          _vtail = (8.0*M_PI*_rho)*(1.0/(9.0*pow(_r_cut,9))-1.0/(3.0*pow(_r_cut,3))); // TO BE FIXED IN EXERCISE 7
        }else _vtail = 0.0;
...
void System :: read_configuration(){
  ...
  cinf.close();
  if(_restart and _sim_type > 1){// if ising simulaton has been restarted, use last spins config. 
    int spin;
...
void System :: istant_pe(double pe){ // Reset block accumulators to zero
  ofstream write;
    write.open(output_path+"OUTPUT/istant_pe.dat",ios::app);
    write << pe << endl;
    write.close();
  return;
}
...
void System :: measure(){ // Measure properties
  ...
  double tenergy_temp=0.0; // temporary acc for total energy
  double magnetization=0.0;// acc for magn
  double virial=0.0;// virial for pressure
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if(_measure_gofr){
          r = 0.0;
          for( bin = 0; bin<_n_bins; bin++){
            if(dr>r and dr<=r+_bin_size) _measurement(_index_gofr+bin)+=2.;
            r +=_bin_size;
          }
        }
        if(dr < _r_cut){
          ...
          //////////////////////////////////////////////////////////////////////// PRESSURE
          if(_measure_pressure) virial += (1.0/(3.0*_volume))*(48.0/pow(dr,12) - 24.0/pow(dr,6));// this is w/v
        }
      }
    }
  }
  ...
  if (_measure_penergy){
    ...
    ////////////////////////////////////////////////////////////////////////////Ex 7 autocorrelation
    //this->istant_pe(penergy_temp); 
  ...
  if (_measure_tenergy){
    if (_sim_type < 2) _measurement(_index_tenergy) = kenergy_temp + penergy_temp; // for molecular D.
    else { //for spin system
      double s_i, s_j;
      for (int i=0; i<_npart; i++){
  ...
  // PRESSURE //////////////////////////////////////////////////////////////////
  if(_measure_pressure and _measure_temp) _measurement(_index_pressure) = _rho*_measurement(_index_temp)+virial+_ptail;
  // GOFR //////////////////////////////////////////////////////////////////////
  if(_measure_gofr) {//gofr normalizz
    r = 0.0;
    double dv_r;
    for( bin = 0; bin<_n_bins; bin++){
      dv_r = (4.0/3.0)*M_PI*(pow(r+_bin_size,3.)-pow(r,3.));
       _measurement(_index_gofr+bin)/= dv_r*_npart*_rho;
      r +=_bin_size;
    }
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv and _measure_tenergy) _measurement(_index_cv) = tenergy_temp*tenergy_temp;
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi){
    double sum_s_i = 0.0;
    double chi;
    ...
    chi = _beta*((sum_s_i*sum_s_i)/double(_npart));
...
void System :: averages(int blk){
  ...
  _average     = _block_av / double(_nsteps); // averages of one block over nsteps 

  // SPECIFIC HEAT (to compute actual cv with expt values of penergy) ///////////
  if (_measure_cv and _measure_tenergy){
    //_average(_index_cv) contains exp value of int energy squared
    double cv ;
    cv = _beta*_beta*(_average(_index_cv)-_average(_index_tenergy)*_average(_index_tenergy));
    _average(_index_cv) = cv;
  }
  _global_av  += _average; // accumolator for averages of the blocks
  ...
  if (_measure_gofr){
    ...
    double r = 0.0;
    if(blk == _nblocks){// print gofr only at the end of simulation
      for(int bin = 0; bin<_n_bins; bin++){
        average  = _average(_index_gofr + bin);
  ...
  coutf << scientific << setw(12) << blk << setw(16) << fraction << endl;
  coutf.close();
