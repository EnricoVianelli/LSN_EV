...
private:
  string output_path;     // path were to print all the OUTPUT
  ...
  double _J, _H;        // Parameters for the Ising Hamiltonian ex 6
  ...
  vec _block_av;         // Single Block averages of properties over nstep
  vec _global_av;        // Global averages of properties over the number of blocks
  vec _global_av2;       // Squared global averages of properties
  vec _average;          // Average values of properties over one step
  vec _measurement;      // vector with Measured VALUES of properties 

public: // Function declarations
  int get_nbl();              // Get the number of blocks
  int get_nsteps();           // Get the number of steps in each block
  void initialize(string path);          // Initialize system properties
