...
int main(int argc, char *argv[]) {
  ...
  //------------Random Walk in cubic lattice---------------------------
  double a = 1.; // lattice constant
  double index; // index of the coordinate that makes a step (x,y or z)
  int M = 100000;
  int N = 100;
  int steps = 100;
...
//---------------------main loop M random walks--------------------------
  for(int m=0; m< M; m++){
        ...
        for(int i=1 ; i<steps+1;i++){
            index = rnd.Dice(0,3);// chosing arb the coord to change
            r[index] = r[index] + 2*a*(double(rnd.Head_Cross())-0.5);// one step
...
// ---------------Blocks statistics and saving data---------------
  ofstream WriteData;
  WriteData.open("Positions_RW1_try.out");
