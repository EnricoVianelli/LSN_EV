...
public:
  ...
  // Random number following a Exponential distribution
  double Expo(double lambda);
  // Random number following a Cauchy-Lorentz distribution
  double Lorentz(double mean, double gamma);
  //Head Cross distribuition
  int Head_Cross(void);
  // random int number btw min-max
  int Dice(int min, int max);
// Accept reject for any given probability function
//double AR(function<double()> func);
};
